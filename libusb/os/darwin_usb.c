...
static int darwin_claim_interface(struct libusb_device_handle *dev_handle, uint8_t iface) {
  ...
  if (!usbInterface) {
    usbi_err (ctx, "interface not found");
    return LIBUSB_ERROR_NOT_FOUND;
  }
  ...
  if (kresult != kIOReturnSuccess) {
    usbi_err (ctx, "USBInterfaceOpen: %s", darwin_error_str(kresult));
    return darwin_to_libusb (kresult);
  }
...
static int darwin_detach_kernel_driver (struct libusb_device_handle *dev_handle, uint8_t interface) {
  ...
  if (dpriv->capture_count == 0) {
    ...
    if (!darwin_has_capture_entitlements ()) {
      usbi_warn (ctx, "no capture entitlements. can not detach the kernel driver for this device");
      return LIBUSB_ERROR_NOT_SUPPORTED;
    }
    ...
    if (kresult != kIOReturnSuccess) {
      usbi_err (ctx, "IOServiceAuthorize: %s", darwin_error_str(kresult));
      return darwin_to_libusb (kresult);
    }
